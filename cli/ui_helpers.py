...
def _prompt_visual_mode() -> Optional[str]:
    choice = prompt_choice(
        "请选择画面模式",
        [
            "纯配图（AI图像）",
            "纯YouTube视频剪辑",
            "混合模式（默认配图，可编辑media_plan.json）",
        ],
        default_index=0,
    )
    if choice is None:
        return None
    if choice.startswith("纯YouTube"):
        return "youtube"
    if choice.startswith("混合模式"):
        return "hybrid"
    return "image"
    ...
    while True:
        ...
        if entry.startswith("新建项目"):
            ...
            if mode is None:
                print("👋 返回上一级")
                continue
            visual_mode = _prompt_visual_mode()
            if visual_mode is None:
                print("👋 返回上一级")
                continue
            run_mode = "auto" if mode.startswith("全自动") else "step"
            return {"entry": "new", "input_file": input_file, "run_mode": run_mode, "visual_mode": visual_mode}
        ...
        return {
            ...
            "selected_step": step_val,
            "image_method": progress.get('image_method'),
            "visual_mode": progress.get('visual_mode'),
        }
...
def _run_specific_step(
    ...
    image_server, image_model, image_size, video_size, image_style_preset, opening_image_style, images_method, visual_mode,
    ...
    if target_step == 1.5:
        ...
        if result.get("success") and visual_mode in {"youtube", "hybrid"}:
            media_plan_path = result.get("media_plan_path")
            if media_plan_path:
                print(f"💡 已生成可编辑素材计划文件: {media_plan_path}")
    elif target_step == 3:
        ...
        selection = _prompt_segment_generation_scope(
            project_output_dir,
            step_label="素材" if visual_mode in {"youtube", "hybrid"} else "图像",
            opening_label="开场素材" if visual_mode in {"youtube", "hybrid"} else "开场图",
        ...
        if selection["mode"] == "partial":
            result = run_step_3(
                ...
                image_style_preset=selected_style,
...
def run_cli_main(
    ...
    try:
        ...
        visual_mode = params.get("visual_mode", "image")
    ...
    try:
        ...
        if visual_mode in {"image", "hybrid"}:
    ...
    if input_file is None:
        ...
        if selection["entry"] == "new":
            ...
            visual_mode = selection.get("visual_mode") or visual_mode
        else:
            # 处理已有项目的步骤执行循环
            ...
            return _run_step_by_step_loop(
                ...
                opening_image_style, images_method, visual_mode, tts_server, voice, speech_rate, loudness_rate,
