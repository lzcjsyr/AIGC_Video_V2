...
- **输入**：分段脚本数据
- **处理**：
  - `image`模式：按 `keywords/description` 生成图像所需数据
  - `youtube/hybrid`模式：额外生成 `media_plan.json`（含每段YouTube链接与裁剪区间，可手工编辑）
- **输出**：`keywords.json` / `mini_summary.json` / `media_plan.json`

### 步骤3：素材生成 - 配图/YouTube剪辑

- **输入**：脚本 + 步骤2产物
- **处理**：
  - `image`：AI配图
  - `youtube`：下载并裁剪YouTube片段
  - `hybrid`：按 `media_plan.json` 的 `source` 字段逐段选择来源
- **输出**：`media/opening.*` + `media/segment_N.(png/mp4)`
- **支持**：指定段落重跑、编辑 `media_plan.json` 后再生成
...
- 步骤3后：可重跑素材生成（配图/YouTube）或重新生成指定段落
...
YOUTUBE_SEARCH_TOP_K = 8                               # youtube/hybrid 模式候选数量
