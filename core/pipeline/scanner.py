...
def detect_project_progress(project_dir: str) -> Dict[str, Any]:
    ...
    media_plan = _read_json_if_exists(paths.media_plan_json())
    visual_mode = str((media_plan or {}).get("visual_mode") or "image").strip().lower()
    ...
    if has_script:
        try:
            ...
            # 素材检查 - 使用 ProjectPaths（兼容图片/视频）
            media_files = [f for f in os.listdir(paths.media) if os.path.isfile(os.path.join(paths.media, f))] if os.path.isdir(paths.media) else []
            image_indices = []
            for f in media_files:
                m = re.match(r'^segment_(\d+)\.(png|jpg|jpeg|mp4|mov|avi|mkv|webm|m4v|flv)$', f, re.IGNORECASE)
                if m:
...
def collect_ordered_assets(project_dir: str, script_data: Dict[str, Any], require_audio: bool = True) -> Dict[str, List[str]]:
    ...
    for i in range(1, num_segments+1):
        ...
        candidates = [
            ...
            os.path.join(paths.media, f"segment_{i}.jpg"),
            os.path.join(paths.media, f"segment_{i}.jpeg"),
            os.path.join(paths.media, f"segment_{i}.mp4"),
            os.path.join(paths.media, f"segment_{i}.mov"),
            os.path.join(paths.media, f"segment_{i}.avi"),
            os.path.join(paths.media, f"segment_{i}.mkv"),
            os.path.join(paths.media, f"segment_{i}.webm"),
            os.path.join(paths.media, f"segment_{i}.m4v"),
            os.path.join(paths.media, f"segment_{i}.flv"),
        ]
        image_path = None
        for p in candidates:
            if os.path.exists(p):
        ...
        if not image_path:
            raise FileNotFoundError(f"缺少素材: segment_{i}.(png|jpg|jpeg|mp4|mov|avi|mkv|webm|m4v|flv)")
...
def clear_downstream_outputs(project_dir: str, from_step) -> None:
    ...
    try:
        if from_step <= 1:
            ...
            if os.path.exists(paths.media_plan_json()):
                os.remove(paths.media_plan_json())
                    
        if from_step <= 2:
            if os.path.exists(paths.media_plan_json()):
                os.remove(paths.media_plan_json())
            # 清空 media
            if os.path.isdir(paths.media):
                for f in os.listdir(paths.media):
                    fp = os.path.join(paths.media, f)
