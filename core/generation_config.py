...
class VideoGenerationConfig:
    ...
    num_segments: int = 6
    visual_mode: str = "image"  # image / youtube / hybrid
    ...
    def __post_init__(self):
        """初始化后的验证和默认值设置"""
        self.visual_mode = (self.visual_mode or "image").strip().lower()
        if self.visual_mode not in {"image", "youtube", "hybrid"}:
            self.visual_mode = "image"
        # 设置默认值
...
class StepExecutionConfig:
    ...
    def from_generation_config(
        ...
        return cls(
            ...
            visual_mode=gen_config.visual_mode,
