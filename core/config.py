...
VISUAL_MODE = "image"                               # 画面模式: image / youtube / hybrid

# YouTube检索与裁剪参数
YOUTUBE_SEARCH_TOP_K = 8                              # 每段检索候选数
YOUTUBE_MIN_DURATION_SECONDS = 6                      # 候选最短时长
YOUTUBE_MAX_DURATION_SECONDS = 180                    # 候选最长时长
YOUTUBE_DEFAULT_CLIP_DURATION_SECONDS = 6.5           # 回退截取时长
YOUTUBE_OPENING_DEFAULT_DURATION_SECONDS = 6.5        # 开场默认截取时长
...
_USER_CONFIG_ATTRS = [
    # 画面模式与YouTube参数
    'VISUAL_MODE',
    'YOUTUBE_SEARCH_TOP_K', 'YOUTUBE_MIN_DURATION_SECONDS', 'YOUTUBE_MAX_DURATION_SECONDS',
    'YOUTUBE_DEFAULT_CLIP_DURATION_SECONDS', 'YOUTUBE_OPENING_DEFAULT_DURATION_SECONDS',
    # 音视频参数
...
Config.SUPPORTED_VISUAL_MODES = ["image", "youtube", "hybrid"]
...
def _validate_parameters_impl(
    ...
    if visual_mode not in cls.SUPPORTED_VISUAL_MODES:
        raise ValueError(f"不支持的画面模式: {visual_mode}，支持的模式: {cls.SUPPORTED_VISUAL_MODES}")
    ...
    if visual_mode in {"image", "hybrid"} and (not cls.validate_image_size(image_size, image_model)):
        raise ValueError(
